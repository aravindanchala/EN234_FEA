...
!
       ...
       double precision E,xnu,Y,e0,m,n,edot0,S,H,td,O,alpha
       double precision f,dfde,ds0de,dCde,dtade,s0,x,err,tol,C
...
!     Elastic increment (either no stress, or no plastic strain rate)
       ...
       do while (err>tol)
           ...
           dta = (dt/(x+exp(-1.d0*x)) )+ 
     1                (ta*(exp(-1.d0*x)-1)/(1+(exp(-1.d0*x)/x)))
          endif 
           
           C = 1 - exp(-1.d0*(((ta+dta)/td)**alpha))
           f = (sestar/S) - (1.5d0*E*deplas/(1.d0+xnu)/S) - (s0/S)-(H*C)
     1                    - log(deplas/(dt*edot0))
           
           ds0de = (m*Y/e0)*(1+(eplas+deplas/e0))**(m-1)
           dtade = dt*(-1.d0/(x + exp(-1.d0*x))**2)*((1/O)-
     1                (exp(-1.d0*x)/O)) + ta*(-1.d0/O)*(exp(-1.d0*x)) 
           
           dCde =  exp(-1.d0*(((ta + dta)/td)**alpha))*(1.d0/td)*(dtade)
     1                      *(alpha)*(((ta+dta)/td)**(alpha-1))
           
           dfde = -1.5d0*E/((1.d0+xnu)*S) - ((ds0de/S) + (H*dCde) 
     1               + (1.d0/deplas))
        ...
        stressNew(k,1:ntens) = (1.d0 - 1.5d0*deplas*E/(1.d0+xnu)/sestar)
     1                          *sdevstar(1:ntens)
        stressNew(k,1:ndir) = stressNew(k,1:ndir) + skkstar/3.d0
