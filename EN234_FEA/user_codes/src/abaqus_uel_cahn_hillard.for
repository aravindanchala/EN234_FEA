...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
      ...
      double precision  ::  De(3,3)                            ! stress = D*(strain)  (NOTE FACTOR OF 2 in shear strain)
      ...
      double precision  ::  E, xnu, D44, D11, D12, O , WW ,k, diff,theta   ! Material properties
      double precision  ::  c,f,s_33,s_kk,dfdc                                  ! concentration
    ...
    !     PROPS(2)         Poisson's ratio
      ...
      WW = PROPS(4)
      k = PROPS(5)
      diff = PROPS(6) ! Same as diffusion Coefficient D
      theta = PROPS(7)
      D44 = 0.5D0*E/(1+xnu)
      ...
      dNbardx(1:4,1:2) = matmul(dNbardxi(1:4,1:2),dxidx)
        ...
        c = sol(5)
        f = 2*WW*c*(c-1)*(2*c-1)
          dfdc = WW*(12*(c)*(c) -12*(c)+2)
        ! D33 = D11 , D31 = D32 = D12
        s_33 = D12*sol(1)+ D12*sol(2)-(D11+2*D12)*O*c/3
        
           
        q = 0.d0
        q(1:3) = matmul(De,sol(1:3))      
        s_kk = q(1) + q(2)+ s_33
          
        q(4) = sol(4) - f - O*s_kk/3
        q(5) = dsol(5)/DTIME
        q(6) = -k*sol(8)
        q(7) = -k*sol(9)
        ...
        D(4,1) = -1.d0*O*(D11 + D12 + D12)/3
        D(4,2) = -1.d0*O*(D11 + D12 + D12)/3
        D(4,4) = 1.d0
        D(1,5) = D(4,1)
        D(2,5) = D(4,2)
        D(5,5) = 1.d0/DTIME
        ...
        D(8,6) = theta*diff
        D(9,7) = theta*diff
        D(4,5) = -1.d0*dfdc - O*O*(2*D12+ D11)/3
