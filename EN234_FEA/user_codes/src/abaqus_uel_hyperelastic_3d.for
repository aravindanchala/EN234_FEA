...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
      ...
      double precision  ::  K, mu, G11 , G22, G33, G44        ! Material properties
      ...
      double precision  ::  Sig(3,3)                          ! Material Stress matrix Sigma 
      double precision  ::  G(6,6)                            ! Anisotropic elastic modulii
      double precision  ::  H(6,9)                            ! H in the stiffness matrix 
      

    !
    !     Example ABAQUS UEL implementing 3D linear hyperelastic elements
    !     El props are:

    !     PROPS(1)         Shear modulus
    !     PROPS(2)         Bulk Modulus
      !   PROPS(3)         G11
    !     PROPS(4)         G22
      !   PROPS(5)         G33
    !     PROPS(6)         G44
    ...
    !
     ...
     1                 - N2(1:nfacenodes)*adlmag(j,1)*norm(1:3)*w(kint)      ! Note determinant is already in normal
      ...
      subroutine abq_UEL_3D_material_T_matrix(F,G,K,mu,Sig,D)
      ...
      double precision, intent(out) :: Sig(3,3)
      double precision, intent(out) :: D(6,6)
      
      double precision  ::  C(3,3)                            ! F*Ftranspose
      double precision  ::  C_det                             ! Determinant of C
      double precision  ::  Cvec(6)                           ! C in vector form
      double precision  ::  Cbarvec(6)                        ! Cvec/J^(2/3)
      double precision  ::  Cinv(3,3)                         ! Inverse of C 
      double precision  ::  Cinvvec(6)                        ! Vector of Inverse of C 
      double precision  ::  Pvec(6)                           ! First Piola kirchoff  vector 
      double precision  ::  Cstarvec(6)                       ! twice the shear elements 
      double precision  ::  Cstarbarvec(6)                    ! Cstarvec/J^(2/3)
      double precision  ::  CstarbarI(6)                    ! Cstarbarvec - I 
      double precision  ::  I(6)                            ! Identity vector
      double precision  ::  Q                                 ! Q from exp(Q)
      double precision  ::  Sigvec(6)                            ! Sigma Vector 
      double precision  ::  O(6,6)                            ! Omega Matrix 
      
      
      
      C = 0.d0 
      C = matmul(F,transpose(F)) ! Equivalent to C in HW 
      call abq_UEL_invert3d(F,Finv,JJ)
      ...
      ! Assembling Cbar vector 
      Cbarvec = 0.d0
      do i = 1,6
      Cbarvec(i) = Cvec(i)*(1/JJ^(2/3))
      end do 
      
      call abq_UEL_invert3d(C,Cinv,C_det)
      
      ! Assembling Cinvvec vector 
      Cinvvec = 0.d0 
      Cinvvec(1) = Cinv(1,1)
      Cinvvec(2) = Cinv(2,2)
      Cinvvec(3) = Cinv(3,3)
      ...
      ! Assembling Cbar star vector 
      Cstarbarvec = 0.d0
      do i = 1,6
      Cstarbarvec(i) = Cstarvec(i)*(1/JJ^(2/3))
      end do 
      ...
      I(i) = 1.d0
        ...
        ! Assembling CbarstarI 
      CstarbarI = 0.d0
      do i = 1,6
      CstarbarI(i) = Cstarbarvec(i) - I(i)
      end do     
      
      
          Pvec(1:6) = 0.5d0*(1/JJ^{2/3})*(matmul(G,CstarbarI(1:6))
     1  -(1/3)*matmul(matmul(Cstarvec(1:6),matmul(G,CstarbarI(1:6)))
     2                ,Cinvvec(1:6))
          
          
      Q =0.d0
      Q = 0.25d0*(matmul(transpose(CstarbarI(1:6)),
     1 matmul(G,CstarbarI(1:6))))
      
      Sigvec = 0.d0 
      Sigvec(1:6) = mu*exp(Q)*Pvec(1:6) + K*JJ*(JJ-1)*Cinvvec(1:6)
      
      ! Assembling Omega Matrix 
      O =0.d0
      ...
      end subroutine abq_UEL_3D__material_T_matrix
